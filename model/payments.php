...
function load_detail_bill($ma_hd)
{
    $sql = "Select * from `payments` join `detail_bills` on `payments`.`stripe_charge_id`=`detail_bills`.`ma_hd` 
    where `payments`.`stripe_charge_id`= '$ma_hd'";
    $bills = pdo_query($sql);
    return $bills;
}
?>